...
#  Detailed citation will be listed directly above the borrowed code.

#=============================================================================================================
#=============================================================================================================
# Below code is from # Jags-Ymet-XmetMulti-MrobustVarSelect.R and Jags-Ydich-XmetMulti-Mlogistic.R, Jags-Ydich-XmetMulti-MlogisticRobust.R, with some modifications
#=============================================================================================================
...
genMCMC = function( data , xName, yName, 
  ...
  #-----------------------------------------------------------------------------
  # THE MODEL.
  # Some of the model string below is borrowed from 
  # https://stackoverflow.com/questions/38291082/logistic-regression-in-jags
...
#===================================================================================================
#===================================================================================================
# Below are variable selection part, copied from Jags-Ymet-XmetMulti_MrobustVarSelect.R
#===================================================================================================
#===================================================================================================
#===================================================================================================
#===================================================================================================
...
genMCMC_VarSelect = function( data , xName="x" , yName="y" , 
                    numSavedSteps=10000 , thinSteps=1 , saveName=NULL ,
                    runjagsMethod=runjagsMethodDefault , 
                    nChains=nChainsDefault ) { 
  ...
  model {
    ...
    for ( i in 1:Ntotal ) {
      y[i] ~ dbern( mu[i] )
      mu[i] <- ilogit(zbeta0 + sum( delta[1:Nx] * zbeta[1:Nx] * zx[i,1:Nx] ) )
    }
    ...
    # Transform to original scale:
    beta[1:Nx] <- zbeta[1:Nx] / xsd[1:Nx] 
    beta0 <- zbeta0 - sum( delta[1:Nx] * zbeta[1:Nx] * xm[1:Nx] / xsd[1:Nx] )
  }
  " # close quote for modelString
  # Write out modelString to a text file
  writeLines( modelString , con="TEMPmodel_VarSelect.txt" )
  #-----------------------------------------------------------------------------
  # INTIALIZE THE CHAINS.
  # Let JAGS do it...
  ...
  #-----------------------------------------------------------------------------
  # RUN THE CHAINS
  parameters = c( "beta0" ,  "beta" ,  "delta" , "zbeta0" , "zbeta" 
                  # , theta # estimate prior inclusion probability
  )
  adaptSteps = 500  # Number of steps to "tune" the samplers
  burnInSteps = 1000
  runJagsOut <- run.jags( method=runjagsMethod ,
                          model="TEMPmodel_VarSelect.txt" , 
                          monitor=parameters , 
                          data=dataList ,  
                          #inits=initsList , 
                          n.chains=nChains ,
...
smryMCMC_VarSelect = function(  codaSamples , 
                                saveName=NULL ) {
  summaryInfo = NULL
  mcmcMat = as.matrix(codaSamples)
  paramName = colnames(mcmcMat)
...
plotMCMC_VarSelect = function( codaSamples , data , xName="x" , yName="y" ,
                               showCurve=FALSE ,  pairsPlot=FALSE ,
                               saveName=NULL , saveType="jpg" ) {
  # showCurve is TRUE or FALSE and indicates whether the posterior should
  #   be displayed as a histogram (by default) or by an approximate curve.
  ...
  if ( ncol(x)==1 ) {
    ...
    for ( cIdx in cVec ) {
      lines( xComb , 
            1/(1+exp(-(beta0[cIdx]+beta[cIdx,1]*xComb ))) , 
             lwd=1.5 ,
             col="skyblue" )
      xInt = -beta0[cIdx]/beta[cIdx,1]
      arrows( xInt,0.5, xInt,-0.04, length=0.1 , col="skyblue" , lty="dashed" )
